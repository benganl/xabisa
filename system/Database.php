...
class Database
{
    ...
    public static function getInstance(string $username, string $password): self
    {
        if (self::$instance === null) {
            self::$instance = new self();
            self::$instance->connect($username, $password);
        }
    ...
    private function connect(string $username, string $password): void
    {
        if ($this->dbh === null) {
            try {
                $this->dbh = new PDO('mysql:host=localhost;dbname=wyzecmsdb;charset=utf8', $username, $password);
